...
const DrawShapeKLineChart: React.FunctionComponent<{}> = () => {
  const [kLineChart, setKlineChart] = useState<Chart | null>();
  useEffect(() => {
    let k: Chart | null = init("draw-shape-k-line");
    setKlineChart(k);
    if (k) {
      // k.addShapeTemplate([rect, circle]);
...
export default DrawShapeKLineChart;
