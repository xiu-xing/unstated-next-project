...
const CommonChart: React.FunctionComponent<CommonChartProps> = props => {
  ...
  function initTooltip(chartConfig: ChartModel, hasData: boolean) {
    ...
    if (chartConfig.series.length > 1) {
      ...
      if (getOptionType(chartConfig.series[0]) == OptionType.candlestick) {
        return {
          ...commonTooltip,
          axisPointer: {
            type: "cross",
  ...
  function initDataZoom(chartConfig: ChartModel, hasData: boolean) {
    ...
    if (chartConfig.dataZoom && chartData.dataset.length > 10) {
      ...
      if (getOptionType(chartConfig.series[0]) == OptionType.candlestick && chartData.dataset.length > 30) {
        return {
          type: "inside",
          start: 96,
          end: 100,
      ...
      if (chartConfig.dataZoomShowCount && chartData.dataset.length > chartConfig.dataZoomShowCount + 1)
        dataZoom = _.merge(dataZoom, { startValue: chartData.dataset.length - (chartConfig.dataZoomShowCount + 1) });
      return dataZoom;
    }
  }
  ...
  function initLegend(chartConfig: ChartModel, hasData: boolean): LegendOption | undefined {
    ...
    if (chartConfig.series.length > 1) {
      ...
      const min = Math.min(series.length, chartData.dataset?.[0]?.length - 1);
      const legendData: { name: string; icon?: string }[] = [];
      for (let i = 0; i < min; i++) {
        const type = getOptionType(chartConfig.series[i]);
        const seriesName = chartConfig.series[i].name;
        if (type == OptionType.bar || type == OptionType.candlestick) {
          legendData.push({
            name: seriesName ?? chartData.dataset[0][i + 1].toString() ?? "",
          });
        }
        if (type == OptionType.line) {
          legendData.push({
